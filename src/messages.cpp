...
messages::MessageHandler::MessageHandler() {
  _fileHandler = std::make_unique<messages::FileHandler>();
}

messages::MessageHandler::MessageHandler(std::string keyPair,
 ...
 */
void messages::MessageHandler::addUserInfo(
    int *size, payload::packet *packet, p2p::myInfo const &myInfo,
  ...
  *size = packet->ByteSize() + 4;
   ...
   */
  bool messages::MessageHandler::deserializeMessage(
      payload::packet * packet, char *buffer, uint32g size) {
    ...
    // Once the embedded message has been parsed, PopLimit() is called to undo
    // the limit
   ...
   * @param[in, out] packet protobuf packet, containing payload to be
   * deserialized
   */
  bool messages::MessageHandler::readBody(int sock, uint32g size,
                                          payload::packet *packet) {
    int bytecount;
    char buffer[size + 4];
   ...
   */
  void messages::MessageHandler::sendMessage(int size, int sock,
                                             payload::packet &packet) {
   ...
   */
  bool messages::MessageHandler::receiveMessage(int sock,
                                                payload::packet *packet) {
    char buffer[4];
    int bytesIn;
    memset(buffer, '\0', 4);
  ...
  bool messages::MessageHandler::authenticate(int sock) {
    ...
    if (!this->receiveMessage(sock, &packet)) {
      std::cout << "[AUTHENTICATE] Could not authenticate. Connection closed "
                   "by other party"
                << std::endl;
      return false;
    }
    ...
    if (!this->receiveMessage(sock, &packet)) {
      std::cout << "[AUTHENTICATE] Could not successfully authenticate."
                << std::endl;
      return false;
    }
    ...
    std::cout
        << "[AUTHENTICATE] We have successfully authenticated with other party!"
        << std::endl;
    return true;
  }
  ...
  bool messages::MessageHandler::verify(int sock, std::string &key) {
    ...
    std::string decryptedNonce =
        this->decryptWithPublicKey(encryptedNonce, key);
   ...
   */
  bool messages::MessageHandler::receiveMessage(int socket,
                                                std::string *recvMsg) {
    char buffer[128];
    memset(buffer, 0, 128);
  ...
  /**
   * Sends filePath file sends up to MAX_FILE_SIZE mb as defined in 
   * messages::FileHandler
   * @param sock user socket to send file
   * @param filePath path to file
   */
  bool messages::MessageHandler::sendFile(int sock, std::string &filePath) {
    int size;
    std::string file_name, encoded;
    payload::packet packet;
    auto *payload = packet.mutable_payload();
    auto *filePload = payload->mutable_filepayload();

    // Loads file and base64 encoding
    if (!_fileHandler->loadFile(filePath, &file_name, &encoded)) {
      return false;
    }

    // Send a first message with the file details plus a generated key
    packet.set_time_stamp(messages::MessageHandler::getTimeStamp());
    payload->set_type(payload::packet_MessageTypes_SUCCESS);
    filePload->set_key("lol");
    filePload->set_filenamewithext(file_name);

    size = packet.ByteSize() + 4;
    this->sendMessage(size, sock, packet);

    // Wait for akn from peer before sending the file
    if (!this->receiveMessage(sock, &packet)) {
      return false;
    }

    if (payload::packet_MessageTypes_SUCCESS != payload->type()) {
      return false;
    }
    
    return _fileHandler->sendFile(sock, encoded.c_str(), encoded.length());
  }

  /**
   * Receives file as sent by ::sendFile. Saves name as the given name included
   * by the other peer in the protobuf message
   * @param sock user socket to send file
   */
  bool messages::MessageHandler::receiveFile(int sock){
    std::string file;
    payload::packet packet;
    auto *payload = packet.mutable_payload();
    auto *filePload = payload->mutable_filepayload();

    // Receive key and file name
    if (!this->receiveMessage(sock, &packet)) {
      std::cout << "Could not receive key and file details" << std::endl;
      return false;
    }

    // Get key and file name with extension
    std::string key = filePload->key();
    std::string file_name = filePload->filenamewithext();
    
    // Send ack message to start receiving the file
    payload = packet.mutable_payload();
    packet.set_time_stamp(messages::MessageHandler::getTimeStamp());
    payload->set_type(payload::packet_MessageTypes_SUCCESS);
    int size = packet.ByteSize() + 4;

    this->sendMessage(size, sock, packet);

    if (!_fileHandler->receiveFile(sock, &file)){
      std::cout << "Could not receive file"; 
      return false; 
    }
    
    std::string decoded = _fileHandler->base64decode(file);
    
    return _fileHandler->saveFile(file_name, decoded);
  }
   ...
   * @param hasAdvertisedFirst who initiated the connection - useful for
   * authentication
   */
  payload::packet messages::MessageHandler::setPeerData(
      int *size, std::string &peerIpAddress, std::uint16_t &peerPort,
